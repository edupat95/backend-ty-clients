...
/**
 * REST controller for managing {@link com.tyclients.tycapp.domain.Club}.
 */
@RestController
@RequestMapping("/api")
public class ClubResource {
    ...
    //OBTENER EL CLUB POR ID DE USUARIO ADMINISTRADOR
    @GetMapping("/clubs/user/{user_id}")
    public Optional<Club> getClubByIdUser(@PathVariable Long user_id) {
        log.debug("REST request to get Club By use Id only admin club : {}", user_id);
        Optional<Club> club = clubService.getClubByIdUser(user_id);          
        return club;
    }
}
